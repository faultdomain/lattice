...
/// Create CAPMOX (Proxmox CAPI provider) credentials secret
///
...
///
/// Creates the secret in the capmox-system namespace.
#[cfg(feature = "provider-e2e")]
pub async fn create_capmox_credentials_secret(client: &kube::Client) -> Result<(), String> {
    use k8s_openapi::api::core::v1::{Namespace, Secret};
    use kube::api::{Api, PostParams};
    use std::collections::BTreeMap;

    let url = std::env::var("PROXMOX_URL")
        .map_err(|_| "PROXMOX_URL environment variable required for Proxmox provider")?;
    let token = std::env::var("PROXMOX_TOKEN")
        .map_err(|_| "PROXMOX_TOKEN environment variable required for Proxmox provider")?;
    let secret = std::env::var("PROXMOX_SECRET")
        .map_err(|_| "PROXMOX_SECRET environment variable required for Proxmox provider")?;

    println!("  Creating CAPMOX credentials secret...");
    println!("    PROXMOX_URL: {}", url);
    println!("    PROXMOX_TOKEN: {}", token);

    // Ensure capmox-system namespace exists
    let ns_api: Api<Namespace> = Api::all(client.clone());
    let ns = Namespace {
        metadata: kube::api::ObjectMeta {
            name: Some("capmox-system".to_string()),
            ..Default::default()
        },
        ..Default::default()
    };
    match ns_api.create(&PostParams::default(), &ns).await {
        Ok(_) => println!("    Created capmox-system namespace"),
        Err(kube::Error::Api(e)) if e.code == 409 => {
            println!("    capmox-system namespace already exists")
        }
        Err(e) => return Err(format!("Failed to create capmox-system namespace: {}", e)),
    }

    // Create the credentials secret
    let secret_api: Api<Secret> = Api::namespaced(client.clone(), "capmox-system");
    let mut data = BTreeMap::new();
    data.insert("url".to_string(), k8s_openapi::ByteString(url.into_bytes()));
    data.insert(
        "token".to_string(),
    ...
    data.insert(
        "secret".to_string(),
        k8s_openapi::ByteString(secret.into_bytes()),
    );

    let cred_secret = Secret {
        metadata: kube::api::ObjectMeta {
            name: Some("capmox-manager-credentials".to_string()),
            namespace: Some("capmox-system".to_string()),
            labels: Some(BTreeMap::from([(
                "platform.ionos.com/secret-type".to_string(),
                "proxmox-credentials".to_string(),
            )])),
            ..Default::default()
        },
        data: Some(data),
    ...
    match secret_api
        .create(&PostParams::default(), &cred_secret)
        .await
    {
        Ok(_) => println!("    Created capmox-manager-credentials secret"),
        Err(kube::Error::Api(e)) if e.code == 409 => {
            // Secret exists, update it
            println!("    Updating existing capmox-manager-credentials secret");
            secret_api
                .replace(
                ...
                .await
                .map_err(|e| format!("Failed to update credentials secret: {}", e))?;
        }
        Err(e) => return Err(format!("Failed to create credentials secret: {}", e)),
    }

    println!("  CAPMOX credentials configured successfully");
    Ok(())
}
